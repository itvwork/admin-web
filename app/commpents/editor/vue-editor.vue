...
<div class="vue-editor-wrap" :style='{width:width}'>
  <ul class="vue-editor-nav">
    ...
    <li class="vue-editor-nav-item"><i class="icon">表格</i>
      <div class="vue-editor-table">
        <h4>3*3</h4>
        <ul class="vue-editior-line-table" m>
          <li v-for="(item,index) in table">
              <i v-for="(t,i) in item" :class="{active:t==1}" @mouseover="tableRowCloum(index,i)">{{i}}</i>
          </li>
        </ul>
      </div>
  ...
  <div ref="content" class="content" @click="insertP()" v-html="content" @keydown="key($event)" @input="sendContent()" contenteditable="true" @mouseup="updatePosition" @keyup="updatePosition" :style="{height:editHeight}" @dragover="allowDrop($event)" @drop="drap($event)">
